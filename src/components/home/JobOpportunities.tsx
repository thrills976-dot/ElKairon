            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <AnimatePresence mode="popLayout">
                {filteredJobs.map(job => {
                  ...
                  <motion.div
                    ...
                    key={job.id}
                    className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col h-full relative"
                  >
                    {/* Top badging */}
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className="text-xs font-bold text-navy-400 bg-gray-50 px-2 py-1 rounded border border-gray-100">
                        {job.type}
                      </span>
                    </div>

                    <h4 className="font-display text-xl font-bold text-navy-900 mb-2 leading-tight group-hover:text-teal-600 transition-colors">
                      {job.title}
                    </h4>
                    ...
                    <div className="mt-auto pt-4 border-t border-gray-100">
                      <div className="flex flex-wrap gap-1.5 mb-6">
                        {job.skills.slice(0, 3).map(skill => (
                          <span key={skill} className="text-[10px] font-bold uppercase tracking-wider bg-gray-100 text-gray-600 px-2 py-1 rounded">
                            {skill}
                          </span>
                        ))}
                        {job.skills.length > 3 && (
              ...
              {filteredJobs.length === 0 && (
                <div className="col-span-full py-12 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-300">
                  ...
                  <h4 className="text-lg font-bold text-navy-900 mb-2">No matching jobs found</h4>
                  <p className="text-gray-500 text-sm">Try adjusting your filters or clearing your skill selections to see more opportunities.</p>
                  <button 
                    onClick={() => {
                      setSelectedSkills([]);
                    ...
                    className="mt-4 px-6 py-2 bg-white border border-gray-200 text-navy-900 rounded-lg text-sm font-bold shadow-sm hover:bg-gray-50"
                  >
                    Reset Filters
                  </button>
                </div>
