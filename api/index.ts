...
if (!getApps().length) {
  initializeApp({
    projectId: process.env.FIREBASE_PROJECT_ID || 'ai-studio-elkaironglobalco-aefe260a-36cd-4d79-ae63-13e60becbd01'
  });
}
...
app.post('/api/ai/match-analysis', aiRateLimiter, requireAuth, async (req: Request, res: Response) => {
...
app.post('/api/ai/skill-synonyms', aiRateLimiter, requireAuth, async (req: Request, res: Response) => {
...
app.post('/api/ai/resume-review', aiRateLimiter, requireAuth, async (req: Request, res: Response) => {
...
app.post('/api/chat', chatRateLimiter, requireAuth, async (req: Request, res: Response) => {
  if (!ai) {
  ...
  try {
    ...
    if (context === 'home') {
      ...
      systemInstruction = "You are the Candidate Success Assistant at ElKairon. You help African professionals understand the recruitment process, work permit steps, and how to prepare their dossiers. You also act as a Relocation Assistant to guide candidates on practical destination preparations, arrival planning, and interpreting checklists. CRITICAL INSTRUCTION: You must clearly distinguish between general guidance and official/legal immigration information. Never claim visa approval guarantees, immigration outcomes, legal conclusions, government decisions, or guaranteed flight/accommodation availability. Direct candidates to official authorities for legal matters. You are encouraging, professional, and knowledgeable.";
